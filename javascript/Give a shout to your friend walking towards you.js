/* 
You are walking on the sidwalk, you notice your friend is also walking on the sidewalk.
You need to give a shout to your friend in order to grab their attention.
...
F = Friend
'Shout' phrase:
Since in the example, the difference between the friends is 6
(F's index is 15 & that of Y's is 9), the shout phrase becomes 'Oiii F!'
...
function madShout(sidewalk) {
  ...
  const distance = friend - you
  const timesRepeatI = Math.round(distance / 2)

  return distance <= 2 ? 'Oi F!' : `O${'i'.repeat(timesRepeatI)} F!`
}
