...
const checkParity = (parity, bin) => {
  const check = [...bin].reduce((a, b) => +a + +b, 0)
  if (parity === 'even' && check % 2 === 0) return 0
  if (parity === 'even' && check % 2 !== 0) return 1
  if (parity === 'odd' && check % 2 !== 0) return 0
  if (parity === 'odd' && check % 2 === 0) return 1
}
