...
const createUsername = (name, letter, number) => {
  letter = letter[0]
  const data = CURR_YEAR - number
  return `${name.toLowerCase()}${letter.toLowerCase()}${data}`
}

function addUsername(users) {
  return users.map((user) => {
    const { firstName, lastName, age } = user
    const username = createUsername(firstName, lastName, age)
