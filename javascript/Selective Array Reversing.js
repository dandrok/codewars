...
function selReverse(array, length) {
  const chunkArr = _.chunk(array, length)
  const reverseChunks = chunkArr.map((el) => el.reverse())
  const flatArray = _.flatten(reverseChunks)
  return length < 1 ? array : flatArray
}
...
// function selReverse(array, subArrayLength) {
//   const newArray = Array.from({ length: Math.ceil(array.length/subArrayLength) }, (_, index) =>
//   array.slice(index * subArrayLength, (index + 1) * subArrayLength)
// )
//   const reverseArr = newArray.map(el => el.reverse())

//   return subArrayLength === 0 ? array : flatten(reverseArr)
