...
function driver(data) {
  const [forename, middleName, surname, dateOfBirth, sex] = data

  const timesRepeat9 = Math.max(MIN_LENGTH - surname.length, 0)
  const repeatString9 = '9'.repeat(timesRepeat9)
  ...
  let firstRule = `${surname.slice(0, 5)}${repeatString9}`
  let secondRule = year[2]
  let thirdRule = sex === 'M' ? month : Number(month) + FEMALE_INCREMENT
  let fourthRule = day
  let fifthRule = year[3]
  let sixthRule = `${forename[0]}${middleName ? middleName[0] : NINE}`
  let seventhRule = NINE
  let eighthRule = 'AA'

  return `${firstRule}${secondRule}${thirdRule}${fourthRule}${fifthRule}${sixthRule}${seventhRule}${eighthRule}`.toUpperCase()
}
