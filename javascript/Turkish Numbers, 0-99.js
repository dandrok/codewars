...
const getTurkishNumber = (num) => {
  const first = Math.floor(num / 10) * 10 || numbers['empty']
  const second = String(num / 10).split('.').length > 1 && num % 10

  return `${
    (num === 0 && numbers[num]) || numbers[first] || numbers['empty']
  } ${numbers[second] || numbers['empty']}`.trim()
}
