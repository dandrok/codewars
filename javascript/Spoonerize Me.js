...
separated by a space: words and returns a spoonerism of those words ina string, as in the above example.
A "word" in the context of this kata can contain any of the letters
...
const firstLetter = (word, num) => word[num].charAt(0)
const result = (firstLetter, splitWord, numOfWord) =>
  `${firstLetter}${splitWord[numOfWord].substring(1)}`
const spoonerize = (words) => {
  const splittedWords = words.split(' ')
  const firstWord = firstLetter(splittedWords, 0)
  const secondWord = firstLetter(splittedWords, 1)
  return `${result(secondWord, splittedWords, 0)} ${result(
    firstWord,
    splittedWords,
    1
  )}`
