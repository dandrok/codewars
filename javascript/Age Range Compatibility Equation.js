...
const minAge = (num) =>
  num <= 14 ? Math.floor(num - 0.1 * num) : Math.floor(num / 2 + 7)

const maxAge = (num) =>
  num <= 14 ? Math.floor(num + 0.1 * num) : Math.floor((num - 7) * 2)

const datingRange = (age) => `${minAge(age)}-${maxAge(age)}`
