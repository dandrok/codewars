...
const createDict = (keys, values) =>
  keys.reduce(
    (acc, curr, i) => (acc = { ...acc, [curr]: values[i] ?? null }),
    {}
  )

function createDict(keys, values) {
  let obj = {}
  keys.forEach((el, i) => (obj = { ...obj, [el]: values[i] ?? null }))
  return obj
}
...
const createDict = (keys, values) => {
  ...
  for (let i = 0; i < keys.length; i++) {
    // values[i] ?? null - similar to values[i] === undefined ? null : values[i]
    obj = { ...obj, [keys[i]]: values[i] ?? null }
  }
  return obj
}
