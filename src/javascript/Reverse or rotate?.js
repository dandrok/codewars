...
function revrot(str, sz) {
  ...
  const chunks = Math.floor(l / sz)

  let result = []

  for (let i = 0; i < (chunks * sz); i += sz) {
    const chunk = str.slice(i, sz + i)
    const sum = [...chunk].reduce((acc, curr) => acc + Number(curr), 0)
    if (sum % 2 === 0) {
      ...
      //rest.push(first)
      //result.push(rest)
      result.push(chunk.slice(1) + chunk.at(0))
    }
