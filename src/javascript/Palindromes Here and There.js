...
where all palindromic numbers are replaced with a 1
and all non-palindromic numbers are replaced with a 0.
...
const convertPalindromes = (numbers) => {
  return numbers.map((el) => {
    el = el.toString()
    return el.slice(0, Math.round(el.length / 2)) ===
      [...el.slice(-Math.round(el.length / 2))].reverse().join('')
      ? 1
      : 0
  })
}
