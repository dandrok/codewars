...
function totalLicks(env) {
  ...
  let challengingNum = 0
  let sumOfLicks = 252

  Object.entries(env).forEach((arr) => {
    sumOfLicks += arr[1]
    if (challengingNum <= arr[1]) {
      challengingNum = arr[1]
    }
    if (challengingNum === arr[1] && challengingNum > 0) {
      toughestChallenge = arr[0]
    }
  })
  return `It took ${sumOfLicks} licks to get to the tootsie roll center of a tootsie pop.${
    challengingNum > 0
