...
const textToNum = (text) => Number(text.replace("£", ""));

function fifa({ Home, Away, Draw }, results) {
  const bets = ["Home", "Away", "Draw"];

  const wonSum = results.reduce((acc, curr, i) => {
    const [home, away] = curr.split("-");
    const bet = bets[i];

    if (bet === "Home" && home > away) {
      acc += textToNum(Home);
    }
    if (bet === "Away" && home < away) {
      acc += textToNum(Away);
    }
