...
The discussion gets heated and you are cannot risk favoring either of them as
this might damage your political standing with either of the two clans the samurai generals belong to.
...
and the order of the words need to follow the order of the first occurence of each word in s2.
...
const commonGround = (s1, s2) => {
  ...
  const common = words2
    .flatMap((word) => (words1.includes(word) ? word : []))
    .join(' ')
  return common.length == 0 ? 'death' : common
}
