...
function sc(words) {
  ...
  const formattedWorlds = allWords.map((word) => word.match(/[a-z]/g).join(""));
  const uniqueWorlds = [...new Set(formattedWorlds)];

  return allWords.length - uniqueWorlds.length + uniqueWorlds.join("").length;
}
