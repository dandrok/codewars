...
function longestWord(letters) {
  ...
  const regExp = new RegExp(`[${letters}]`, "g")
  let result = [] || null
  words.forEach(word => word.match(regExp)?.join('') === word && result.push(word))
  const sortedResult = result.sort((a, b) => b.length - a.length)
  const [longestWord] = sortedResult
  return sortedResult.filter(word => word.length === longestWord.length)
}
