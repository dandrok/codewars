...
function startingMark(bodyHeight) {
  ...
  const slope = (maxMark - minMark) / (maxHeight - minHeight);

  // Calculate the constant offset using one of the points
  const constantOffset = minMark - slope * minHeight;

  // Calculate the starting mark for the given bodyHeight
  const startingMark = slope * bodyHeight + constantOffset;
