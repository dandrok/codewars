...
const createFunction = (param, strFun) => {
  return new Function(param, strFun);
};

function runYourString(arg, obj) {
  const paramVal = obj.param;
  const newObj = { [paramVal]: arg, ...obj };
  const fnc = createFunction(paramVal, obj.func);
  return fnc(newObj[paramVal]);
}
