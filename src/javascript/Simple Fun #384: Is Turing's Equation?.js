...
function isTuringEquation(s) {
  ...
  let [firstNum, secondPart] = firstSplit;
  const secondSplit = secondPart.split("=");
  let [secondNum, equal] = secondSplit;
  firstNum = [...firstNum].reverse().join("");
  secondNum = [...secondNum].reverse().join("");
  equal = [...equal].reverse().join("");
  console.log(firstNum, secondNum, equal);
  return Number(firstNum) + Number(secondNum) == equal;
}
