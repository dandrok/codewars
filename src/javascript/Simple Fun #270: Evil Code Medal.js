...
const timeConverter = (time) => time.split(':').join('')

function evilCodeMedal(userTime, gold, silver, bronze) {
  return timeConverter(userTime) < timeConverter(gold)
    ? 'Gold'
    : timeConverter(userTime) < timeConverter(silver)
    ? 'Silver'
    : timeConverter(userTime) < timeConverter(bronze)
    ? 'Bronze'
    : 'None'
}
