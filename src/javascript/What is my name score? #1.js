...
function nameScore(name) {
  const sum = [...name.toUpperCase()].reduce((acc, curr) => {
    if ("ABCDE".includes(curr)) {
      acc += 1;
    } else if ("FGHIJ".includes(curr)) {
      acc += 2;
    } else if ("KLMNO".includes(curr)) {
      acc += 3;
    } else if ("PQRST".includes(curr)) {
      acc += 4;
    } else if ("UVWXY".includes(curr)) {
      acc += 5;
    }
    return acc;
  }, 0);
