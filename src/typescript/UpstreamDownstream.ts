...
const decimal = (num: number): boolean => num % 1 !== 0;
export function time(
  distance: number,
  boatSpeed: number,
  stream: String
): number {
  const [s, val] = stream.split(" ");
  const downstream = distance / (boatSpeed + Number(val));
  const upstream = distance / (boatSpeed - Number(val));
  const result = s.includes("D") ? downstream : upstream;
  return decimal(result) ? Number(result.toFixed(2)) : result;
}
