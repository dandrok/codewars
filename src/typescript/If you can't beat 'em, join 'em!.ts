...
export type Gang = number[];

export const gangStrength = (gang: Gang) =>
  gang.reduce((total, memberStrength) => total + memberStrength, 0);

export function cantBeatSoJoin(gangs: Gang[]): Gang {
  return gangs
    .sort(([...a], [...b]) => gangStrength(b) - gangStrength(a))
    .flat();
}
