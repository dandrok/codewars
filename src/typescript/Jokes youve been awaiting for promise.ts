...
//mport { fetch } from './preloaded';
export async function sayJoke(apiUrl: string, jokeId: number): Promise<{ saySetup: () => string, sayPunchLine: () => string }> {
  ...
  const jokeObj = result.jokes.find((joke: any) => joke.id === jokeId)

  if (!jokeObj) {
    throw new Error(`No jokes found id: ${jokeId}`)
  }

  const { setup, punchLine } = jokeObj
  return {
    saySetup: () => setup,
