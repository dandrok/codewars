...
export function driver(data: Array<string>): string {
  ...
  const surnameSlice = surname.slice(0, 5);
  const surnameSliceLength = surnameSlice.length;
  const slicedSurname =
    surnameSliceLength < 5
      ? surnameSlice + "9".repeat(5 - surnameSliceLength)
      : surnameSlice;
  const birthDateToLocale = new Date(birthDate).toLocaleDateString("en-GB", {
    day: "2-digit",
    month: "2-digit",
    year: "numeric",
  });
  const [date, _time] = birthDateToLocale.split(",");
  const [day, month, year] = date.split("/");
  const birthMonth = gender === "F" ? Number(month) + 50 : month;

  return `${slicedSurname}${year.at(2)}${birthMonth}${day}${year.at(
    3
  )}${name.at(0)}${middleName.at(0) ?? 9}9AA`.toUpperCase();
}
