...
export function solve(s: string): number {
  for (let longest = Math.floor(s.length / 2); longest > 0; longest--) {
    const prefix = s.slice(0, longest);
    const suffixMatchesPrefix = s.endsWith(prefix);
    if (suffixMatchesPrefix) {
      return longest;
    }
  }
  return 0;
