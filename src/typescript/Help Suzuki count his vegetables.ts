...
export function countVegetables(s: string): [number, string][] {
  ...
  const arrayOfElements = s.split(" ");
  const filteredVegetables = arrayOfElements.filter((veggie) =>
    veggies.includes(veggie)
  );
  const uniques = [...new Set(filteredVegetables)];
  let result: [number, string][] = [];
  for (const vegetable of uniques) {
    const filterVegetable = filteredVegetables.filter(
      (veg) => veg === vegetable
    );
    result.push([filterVegetable.length, vegetable]);
  }
